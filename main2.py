...
def prompt_use_proxy():
    """Meminta pengguna untuk memilih apakah menggunakan proxy atau tidak."""
    console.print(Panel(
        "[bold green]Pilih opsi koneksi:[/bold green]\n\n"
        "1. Menggunakan proxy dari file proxy.txt\n"
        "2. Menggunakan IP asli (tanpa proxy)",
        title="Opsi Koneksi",
        style="bold cyan",
        padding=(1, 2)
    ))
    while True:
        try:
            choice = int(input("Masukkan pilihan Anda (1 atau 2): ").strip())
            if choice == 1:
                return True
            elif choice == 2:
                return False
            else:
                print(Fore.RED + "Pilihan tidak valid. Harap masukkan 1 atau 2.")
        except ValueError:
            print(Fore.RED + "Masukkan angka 1 atau 2.")
...
async def call_api(url, data, proxy, token):
    parsed_proxies = parse_proxy(proxy) if USE_PROXY else None
    if USE_PROXY and not parsed_proxies:
    ...
    try:
        response = requests.post(
            ...
            json=data, 
            headers=headers, 
            proxies=parsed_proxies if USE_PROXY else None,
            timeout=30,
...
async def multi_account_mode(all_tokens, all_proxies):
    ...
    for index, token in enumerate(all_tokens, 1):
        ...
        log("INFO", f"Token {index} dengan Proxy: {token_proxies if USE_PROXY else 'IP asli'}", Fore.LIGHTBLUE_EX)
...
async def main():
    ...
    if USE_PROXY:
        ...
        log("INFO", "Anda memilih menggunakan IP asli (tanpa proxy)", Fore.LIGHTBLUE_EX)

    while True:
        try:
            await show_loading_animation("Memuat Aplikasi")
            all_proxies = load_proxies('proxy.txt') if USE_PROXY else []
